...
namespace Entry.Global
{
    public class GameEntry
    {
        ...
        private static async UniTaskVoid RunAsync()
        {
            ...
            {
                await _instance.Run();
                await _instance.InitializeAnalyticsSDK();
            }
            catch (Exception ex)
            {
        ...
        private GameEntry()
        {
            ...
            var analyticsServicePrefab = ResourceLoader.LoadOrThrow<AnalyticsService>("Prefabs/Services/[ANALYTICS_SERVICE]");

            _loadingView = Object.Instantiate(loadingScreenPrefab);
            _analyticsService = Object.Instantiate(analyticsServicePrefab);
        ...
        private async UniTask InitializeAnalyticsSDK()
        {
            try
            {
                AppMetrica.Activate(new AppMetricaConfig("a78d406e-7b77-49b4-8c89-0e9c308c4982")
                {
        ...
        {
            var playerService = _rootContainer.Resolve<GameWorldState>().PlayerState;

            if(playerService.IsFirstLaunch)
                return true;
